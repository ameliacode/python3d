...
silhouette_renderer = MeshRenderer(  # differentiable
...
class Model(nn.Module):
    ...
    def forward(self):
        ...
        image = self.renderer(meshes_world=self.meshes, R=R, T=T)
        loss = torch.sum((image[..., 3] - self.image_ref) ** 2)
        return loss, image


model = Model(meshes=teapot_mesh, renderer=silhouette_renderer, image_ref=image_ref).to(
    device
)
...
for i in tqdm(range(200)):
    ...
    image_pred = phong_renderer(meshes_world=model.meshes.clone(), R=R, T=T)
    image = image_pred[0, ..., :3].detach().squeeze().cpu().numpy()
